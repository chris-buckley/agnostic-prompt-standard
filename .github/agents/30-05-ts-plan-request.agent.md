...
- Prefer stricter diagnostics when already enabled: keep `useUnknownInCatchVariables`, `noImplicitOverride`, `noFallthroughCasesInSwitch`, and `noUncheckedIndexedAccess` on where configured.
...
- Type-only imports: prefer `import type` for types to avoid accidental runtime imports and to reduce circular-dependency risk.
- Promise hygiene: avoid floating promises; `await` or handle rejections explicitly; in event handlers you may use `void someAsync()` with a short comment if intentionally fire-and-forget.
