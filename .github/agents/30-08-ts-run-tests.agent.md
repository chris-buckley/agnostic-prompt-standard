...
# Optional e2e scripts (naming varies by repo)
CMD_SCRIPT_E2E: "npm run e2e"
CMD_SCRIPT_TEST_E2E: "npm run test:e2e"
CMD_SCRIPT_PLAYWRIGHT: "npm run playwright:test"
CMD_SCRIPT_CYPRESS: "npm run cypress:run"
...
# E2E fallbacks
CMD_PLAYWRIGHT: "npx playwright test"
...
# E2E requests (playwright/cypress). Match these before generic "test"/"coverage" handling.
IF USER_MESSAGE matches "(?i)\\b(playwright|e2e|end-to-end)\\b":
  TRY:
    USE `execute` where: command=CMD_SCRIPT_TEST_E2E, explanation="Run e2e tests (repo script: test:e2e)" (atomic)
  RECOVER (e):
    TRY:
      USE `execute` where: command=CMD_SCRIPT_E2E, explanation="Run e2e tests (repo script: e2e)" (atomic)
    RECOVER (e2):
      TRY:
        USE `execute` where: command=CMD_SCRIPT_PLAYWRIGHT, explanation="Run e2e tests (repo script: playwright:test)" (atomic)
      RECOVER (e3):
        TRY:
          USE `execute` where: command=CMD_PLAYWRIGHT, explanation="Run e2e tests (playwright)" (atomic)
        RECOVER (e4):
          TELL "E2E test execution skipped for this request. Run CMD_SCRIPT_TEST_E2E / CMD_SCRIPT_E2E / CMD_PLAYWRIGHT manually after installing deps (CMD_INSTALL_*)."

ELSE IF USER_MESSAGE matches "(?i)\\b(cypress)\\b":
  TRY:
    USE `execute` where: command=CMD_SCRIPT_CYPRESS, explanation="Run e2e tests (repo script: cypress:run)" (atomic)
  RECOVER (e):
    TRY:
      USE `execute` where: command=CMD_CYPRESS, explanation="Run e2e tests (cypress)" (atomic)
    RECOVER (e2):
      TELL "Cypress execution skipped for this request. Run CMD_SCRIPT_CYPRESS / CMD_CYPRESS manually after installing deps (CMD_INSTALL_*)."

ELSE IF USER_MESSAGE matches "(?i)\\b(cov|coverage)\\b":
