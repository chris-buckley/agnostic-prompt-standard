...
USE `runSubagent` where: agentName="10-07-issue-triager", description="Triage", prompt=USER_INPUT
...
IF NEEDS_HISTORY = true:
  ...
  CAPTURE HISTORY_OUT from `runSubagent`
IF NEEDS_WEB = true:
  USE `runSubagent` where: agentName="10-08-issue-web-researcher", description="Web research", prompt=USER_INPUT
...
USE `runSubagent` where: agentName="10-04-issue-evidence-normalizer", description="Normalize evidence", prompt=USER_INPUT
CAPTURE EVIDENCE_OUT from `runSubagent`
USE `runSubagent` where: agentName="10-06-issue-labeler", description="Suggest labels", prompt=USER_INPUT
CAPTURE LABEL_OUT from `runSubagent`
IF NEEDS_DUPES = true:
  USE `runSubagent` where: agentName="10-03-issue-duplicate-analyzer", description="Duplicate analysis", prompt=USER_INPUT
  CAPTURE DUPE_OUT from `runSubagent`
SET DRAFT_INPUT := <DRAFT_INPUT> (from "Agent Inference")
USE `runSubagent` where: agentName="10-02-issue-drafter", description="Draft issue", prompt=DRAFT_INPUT
...
USE `runSubagent` where: agentName="10-01-issue-comment-drafter", description="Draft comment", prompt=COMMENT_INPUT
