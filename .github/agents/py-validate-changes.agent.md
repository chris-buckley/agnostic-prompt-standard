...
You MUST validate changes after edits using repository-aligned tooling when available.
You MUST run pre-commit when configured and available.
...
RECOVER (e):
  TELL "Fast tests skipped or unavailable for this request. Install dev deps with CMD_INSTALL_DEV_UV or CMD_INSTALL_DEV_PIP then run CMD_PYTEST_FAST."
TRY:
