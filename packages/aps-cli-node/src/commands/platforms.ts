import { loadPlatformsDetailed, resolvePayloadSkillDir, type PlatformInfo } from '../core.js';
...
function renderPlatformsTable(platforms: readonly PlatformInfo[]): string {
  ...
  const headerRow = '│' + headers.map((h, i) => padCell(h, widths[i] ?? 0)).join('│') + '│';
...
export async function runPlatforms(): Promise<void> {
  ...
  const { platforms, issues } = await loadPlatformsDetailed(payloadSkillDir);
  const sorted = sortPlatformsForUi(platforms);

  if (issues.length) {
    for (const issue of issues) {
      console.warn(`Warning: platforms/${issue.dirName}: ${issue.message}`);
    }
    console.warn('');
  }
